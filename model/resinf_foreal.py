...
class ResInf(nn.Module):
    def __init__(self, input_plane, seq_len, trans_layers, gcn_layers, hidden_layers, gcn_emb_size, trans_emb_size, pool_type, n_heads, args, input_size=1):
        ...
        # if args.feed_fwd:
        encoder_layer = nn.TransformerEncoderLayer(d_model = self.dim_val, nhead=self.n_heads, batch_first = True, dim_feedforward=self.dim_val)
        # else:
    ...
    def forward(self, x, adj_matrix):
        ...
        x = x.unsqueeze(-1)
        # print(x.shape)
        src1 = self.encoder_input_layer(x)

        src2 = self.positional_encoding_layer(src1)

        src3 = self.encoder(src=src2) # [n_real * (num_nodes + 1), time_ticks, dim_val]
        ...
        if torch.isnan(resilience):
            print('x',x.shape,x)
            print('src1',src1.shape)
            #print('res_emb',res_emb.shape, res_emb)
            #print('between',between_down.shape, between_down)
